...
- **Leftover-remainder indicator could be an actionable nudge.** Today's "X extra serving" label tells the user their cook quantity doesn't divide evenly across household size but doesn't suggest action. A richer version would propose scaling the recipe (e.g., "1 extra serving — scale to 4 or 6?") or surface a one-tap "Scale recipe" button that bumps to the nearest even divisor. Math also needs review: current `cookServings % householdSize` only flags one scenario (recipe doesn't divide cleanly into household-size meals); the planner also schedules multiple leftover meals from one cook event, so the "real" remainder depends on how many leftover slots are scheduled vs cooked. Requires a planner-math investigation before the UX work.
